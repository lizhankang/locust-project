...
class MultiCardPay(HttpUser):
    """
    收银台使用2张礼品卡支付
    """

    def on_start(self) -> None:
        # 是否新建一个钱包用户并绑卡？会不会导致创建的用户太多？
        self.__dict__['info'] = {
    ...
    def test_task1(self):
        ...
        pay_body = {
            ...
            "redeem_cards":
                [
                    {
                        "card_number": "",
                        "amount": self.__dict__.get('info').get('amount') / 2
                    },
                    {
                        "card_number": "",
                        "amount": self.__dict__.get('info').get('amount') / 2
        ...
        with self.client.post(url=card_pay_endpoint, headers=headers, json=pay_body,
