...
class SingleCardPayTaskSet(SequentialTaskSet):
    def on_start(self):
        dev_info = {
            "brand_code": "1024",
            "store_sn": "LPK001",
    ...
    def card_pay_task(self):
        ...
        body = {"scene_type": "MINI", "sub_tender_type": 801, "payer_uid": self.__dict__['payer_uid'], "showModalStatus": True,
                "amount": 1, "redeem_cards": [{"card_number": self.__dict__['card_number'], "amount": 1}], "isClick": True}
        with self.client.post(url=endpoint, headers=headers, json=body,
...
class SingleCardPayTaskUser(HttpUser):
    tasks = [SingleCardPayTaskSet]
