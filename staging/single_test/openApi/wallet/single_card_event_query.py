...
from locust import HttpUser, SequentialTaskSet, task, events, LoadTestShape, FastHttpUser
from locust.runners import WorkerRunner, MasterRunner

from common.auth_utils import AuthUtils


class SingleCardEventTaskSet(SequentialTaskSet):
    def on_start(self):
        self.auth = self.user.__dict__['auth']

    @task
    def task(self):
        endpoint = "/api/wallet/v1/giftcard/members/cards/events"
...
class SingleCardEventUser(FastHttpUser):
    tasks = [SingleCardEventTaskSet]
