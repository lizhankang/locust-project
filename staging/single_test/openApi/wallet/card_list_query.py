...
class CardListQueryTaskSet(SequentialTaskSet):
    def on_start(self):
        self.auth = self.user.__dict__['auth']

    @task
    def task(self):
        endpoint = "/api/wallet/v1/giftcard/members/cards/list"
...
class CardListQueryUser(HttpUser):
    tasks = [CardListQueryTaskSet]
