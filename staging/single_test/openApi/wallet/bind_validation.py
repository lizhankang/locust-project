...
class BindValidationTaskSet(SequentialTaskSet):
    ...
    def task(self):
        ...
        biz_body = {
            "brand_code": "1024",
            "redeem_code": "43788518636704507369"
        }
        ...
        with self.client.post(endpoint, headers=headers, json=body, catch_response=True) as resp:
            if resp.status_code == 200:
                ...
                resp.failure(f"Invalid code: {resp.status_code}")


class BindValidationUser(FastHttpUser):
    tasks = [BindValidationTaskSet]
